...
export class CashBookService {
  ...
  protected convertDateFromClient(cashBook: ICashBook): ICashBook {
    const copy: ICashBook = Object.assign({}, cashBook, {
      transactionDate:
        cashBook.transactionDate != null && cashBook.transactionDate.isValid() ? cashBook.transactionDate.format(DATE_FORMAT) : null
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((cashBook: ICashBook) => {
        cashBook.transactionDate = cashBook.transactionDate != null ? moment(cashBook.transactionDate) : null;
      });
    }
    return res;
  }
